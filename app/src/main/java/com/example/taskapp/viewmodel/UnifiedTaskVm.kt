...
class UnifiedTaskViewModel(app: Application, state: SavedStateHandle? = null) : AndroidViewModel(app) {
    ...
    fun rename(task: Task, newText: String, newDue: String?, rec: Recurrence, newCategoryId: Int) = viewModelScope.launch {
    ...
    fun moveInList(list: List<Task>, from: Int, to: Int) = viewModelScope.launch {
        ...
        current.add(to, itm)
        val resequenced = current.mapIndexed { i, row -> row.copy(pos = i) }
        repo.moveTasks(resequenced)
    }
    fun moveInToDo(list: List<Task>, from: Int, to: Int) = viewModelScope.launch {
        val current = list.toMutableList()
        val itm = current.removeAt(from)
        current.add(to, itm)
        val resequenced = current.mapIndexed { i, row -> row.copy(todoOrder = i) }
        repo.moveTasks(resequenced)
    }
}
